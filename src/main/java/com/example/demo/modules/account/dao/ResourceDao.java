...
public interface ResourceDao {
    ...
    @Insert("insert resource(resource_name, resource_uri, permission) value(#{resourceName},#{resourceUri}, #{permission})")
    ...
    @Select("select * from resource resource left join role_resource roleResource on resource.resource_id = roleResource.resource_id where roleResource.role_id = #{roleId}")
    ...
    @Results(id = "resourceResult", value = {
            ...
            @Result(column = "resource_id", property = "roles",
                    javaType = List.class,
                    many = @Many(select = "com.example.demo.modules.account.dao.RoleDao.getRolesByResourceId"))
    })
